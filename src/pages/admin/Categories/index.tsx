...
import { Meta } from '../commonts/types';
import ProductTable from './CategoryList';
import ModalCreate from './ModalCreate';
import { getListCategories } from './httpClient';
...
const Home = () => {
  ...
  const [searchResult, setSearchResult] = useState<ListCategory[]>([]);
  ...
  const { isFetching } = useQuery({
    ...
    queryFn: ({ queryKey }) => getListCategories(_.omit(queryKey[1] as { [key: string]: any }, 'validate')),
  ...
  return (
    <div>
      ...
      <Box marginBottom="20px" display="flex" justifyContent="space-between">
        ...
        <ModalCreate refetchData={refetchData} />
      </Box>
      ...
      <ProductTable
        ...
        headerRows={['Name', 'Deleted date', 'Actions']}
      />
    </div>
  );
};
