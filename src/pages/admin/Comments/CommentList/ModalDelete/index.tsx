...
import { deleteComment as deleteCommentService } from '../../httpClient';
...
const ModalEdit: React.FC<Props> = ({ row, refetchData }) => {
  ...
  const deleteComment = useMutation({
    mutationFn: () => deleteCommentService(row?._id),
      ...
      >
        <Box sx={style}>
          ...
          <Box sx={{ display: 'flex', justifyContent: 'flex-end', marginTop: '15px' }}>
            <LoadingButton variant="contained" loading={deleteComment.isLoading} onClick={() => deleteComment.mutate()}>
              {t('Submit')}
            </LoadingButton>
          </Box>
        </Box>
