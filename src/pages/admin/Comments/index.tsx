...
import ProductTable from './CommentList';
import { getListComment } from './httpClient';
...
const Home = () => {
  ...
  const [searchResult, setSearchResult] = useState<Comment[]>([]);
  ...
  const { isFetching } = useQuery({
    ...
    queryFn: ({ queryKey }) => getListComment(_.omit(queryKey[1] as { [key: string]: any }, 'validate')),
  ...
  return (
    <div>
      ...
      <ProductTable
        ...
        headerRows={['Name product', 'User', 'Content', 'Rating', 'Actions']}
      />
    </div>
  );
};
