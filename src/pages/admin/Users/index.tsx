...
import ProductTable from './UserList';
import { getListUser } from './httpClient';
...
const Home = () => {
  ...
  const [filter, setFilter] = useState<{ [key: string]: string | number }>({ role: 'user' });
  ...
  const { isFetching } = useQuery({
    ...
    queryFn: ({ queryKey }) => getListUser(_.omit(queryKey[1] as { [key: string]: any }, 'validate')),
  ...
  return (
    <div>
      ...
      <ProductTable
        ...
        headerRows={['Name', 'Email', 'Address', 'Phone Number', 'Gender', 'Actions']}
      />
    </div>
  );
};
