...
import { deleteProduct as deleteProductService } from '../../httpClient';
...
const ModalEdit: React.FC<Props> = ({ row, refetchData }) => {
  ...
  const deleteProduct = useMutation({
    mutationFn: () => deleteProductService(row?._id),
      ...
      >
        <Box sx={style}>
          ...
          <Box sx={{ display: 'flex', justifyContent: 'flex-end', marginTop: '15px' }}>
            <LoadingButton variant="contained" loading={deleteProduct.isLoading} onClick={() => deleteProduct.mutate()}>
              {t('Submit')}
            </LoadingButton>
          </Box>
        </Box>
